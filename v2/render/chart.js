...
export function initChart() {
  ...
  const resize = () => {
    const c = chartEl;
    if(!c || !S.chart) return;
    // Use chart element's own dimensions (flex:1 min-height:0 guarantees correct size)
    const w = c.clientWidth, h = c.clientHeight;
    if(w > 0 && h > 0) S.chart.applyOptions({ width:w, height:h });
    drawGex();
  };
  // Observe the chart element itself — most accurate
  new ResizeObserver(() => requestAnimationFrame(resize)).observe(chartEl);
  window.addEventListener('resize', () => requestAnimationFrame(resize));
  // Delay first resize to ensure flex layout is complete

...
export function initChart() {
  ...
  const resize = () => {
    const c = chartEl;
    if(!c || !S.chart) return;
    // chartEl is flex:1 min-height:0 — its clientHeight IS the available space.
    // LightweightCharts splits this internally: price pane + time axis.
    // Both are contained within the height we pass — no overflow.
    const w = c.clientWidth, h = c.clientHeight;
    if(w > 0 && h > 0) S.chart.applyOptions({ width:w, height:h });
    drawGex();
  };
  // ResizeObserver on the chart element — fires whenever flex layout changes
  new ResizeObserver(() => requestAnimationFrame(resize)).observe(chartEl);
  window.addEventListener('resize', () => requestAnimationFrame(resize));
  // Triple-rAF: ensures CSS flex layout is fully resolved before measuring
  requestAnimationFrame(() => requestAnimationFrame(() => requestAnimationFrame(resize)));
}
...
export function openFullscreen() {
  ...
  setTimeout(() => {
    ...
    if(c && S.chart) { const hh=c.clientHeight; if(c.clientWidth>0&&hh>0) S.chart.applyOptions({ width:c.clientWidth, height:hh }); }
    drawGex();
  }, 40);
}

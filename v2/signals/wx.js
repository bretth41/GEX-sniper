...
export function computeWX() {
  try {
    ...
    if(!S.wxNeutralBarCount) S.wxNeutralBarCount = 0;
    if(!S.wxLockedDir)       S.wxLockedDir       = 0;
    ...
    if(!S.wxScoreHist) S.wxScoreHist = [];
    S.wxScoreHist.push(S.wxScore);
    if(S.wxScoreHist.length > 3) S.wxScoreHist.shift();

    // GO! requires current score ≥14 AND at least 2 of last 3 bars ≥8
    const recentStrong = S.wxScoreHist.filter(v => v >= 8).length;
...
export function mapWxCompScore() {
  ...
  if(S.wxCompScoreEma === undefined) S.wxCompScoreEma = raw;

...
export function computeG1() {
  ...
  if(!S.g1ScoreHist) S.g1ScoreHist = [];
  S.g1ScoreHist.push({ ts: nowMs, v: rawScore });
  // Prune to 60-min window
  S.g1ScoreHist = S.g1ScoreHist.filter(h => (nowMs - h.ts) <= G1_NORM_WINDOW_MS);

  let finalScore = rawScore;
  if(S.g1ScoreHist.length >= G1_NORM_MIN_BARS) {
    const vals    = S.g1ScoreHist.map(h => h.v);

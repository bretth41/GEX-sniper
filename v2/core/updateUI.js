...
export function updateUI() {
  ...
  // ── KV state save (throttled to every 30s) ───────────────────────
  // Writes signal snapshot to CF KV via Worker POST /state.
  // On tab reload, loadStateFromKV() reads this back instantly.
  _kvSaveThrottle();
}
...
let _kvLastSave = 0;
const KV_SAVE_INTERVAL = 30000; // save every 30s max
...
async function _saveStateToKV() {
  ...
  try {
    const snapshot = {
      ...
      f1Dir:       S.f1Dir,
      f1Status:    S.f1Status,
      f1SweepAccum:S.f1SweepAccum,
      wxState:     S.wxState,
      wxCompScore: S.wxCompScore,
      driftState:  S.driftState,
      driftDelta:  S.driftDelta,
      hecState:    S.hecState,
      hecSlope:    S.hecSlope,
      giaState:    S.giaState,
      giaAccel:    S.giaAccel,
      dscPhase:    S.dscPhase,
      dscScore:    S.dscScore,
      dscVelocity: S.dscVelocity,
      dscDir:      S.dscDir,
      gxState:     S.gxState,
      gxDominantStrike: S.gxDominantStrike,
      gxEmergingStrike: S.gxEmergingStrike,
      cosmicState: S.cosmicState,
      cosmicTarget:S.cosmicTarget,
      spyPrice:    S.spyPrice,
      vixPrice:    S.vixPrice,
      gexLevels:   S.gexLevels,
    };
  ...
  } catch(e) { /* silent — KV save failure never blocks the dashboard */ }
}

export async function loadStateFromKV() {
  // Called on tab load before first fetch cycle.
  // Restores last known signal state so dashboard shows current context
  // instantly rather than blank NEUTRAL while data loads.
  ...
  try {
    ...
    // Only restore if snapshot is less than 8 hours old
    // (older = stale from prior session, not useful)
    const ageHours = (Date.now() - snap.ts) / 3600000;
    if(ageHours > 8) return;

    // Restore key signal fields — live data will overwrite within 5s
    ...
    // Trigger a render with restored state so screen isn't blank
    try { renderHeader(S.liveComp, 0, 0); renderCards(S.liveComp); } catch(e){}

    console.log('[KV] State restored from', new Date(snap.ts).toLocaleTimeString(),
                '— age:', ageHours.toFixed(1)+'h', '— sig:', snap.sigState);
  } catch(e) { /* silent — KV load failure falls back to normal polling */ }
}
